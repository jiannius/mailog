...
The package's reason for being. A singleton listener subscribes to Laravel's `MessageSending` and `MessageSent` events. On *sending* it builds a `PENDING` `MailLog` from the Symfony `Email` (sender, recipients, subject, both bodies, attachment metadata, tags, metadata, mailer, mailable) plus the resolved user/data, and records `message-object => log id` in a **`WeakMap`** (object-keyed so it can't leak or mis-correlate via `spl_object_id` reuse in long-running workers). On *sent* it flips the row to `SENT` with the transport message id. If the transport throws at send time, the `MailLogTransport` decorator (see below) calls the listener's `failed()` to flip the row to `FAILED` with the error message + `failed_at`, then rethrows so the host's send fails as normal. A row left `PENDING` therefore means the send never completed (e.g. the worker died mid-send). All handlers are wrapped in `try/catch` + `report()` — **logging must never break the host's email send**.

### Send-failure capture (`src/Transport/MailLogTransport.php`)

Laravel fires **no event** for a failed send (only `MessageSending` / `MessageSent`), so the only place to observe a synchronous transport error is around the transport's `send()`. `MailLogTransport` is a thin decorator implementing Symfony's `TransportInterface`: it forwards `send()` to the inner transport, and on a thrown exception flips the pending row to `FAILED` (via `MailLogListener::failed()`, looked up through the same `WeakMap` by the message object — the *same* `Email` instance Laravel passes to both the event and the transport) before rethrowing. `__call` forwards any other method (e.g. `ArrayTransport::messages()`) to the inner transport so the decorator stays transparent. It is installed in `boot()` by `captureSendFailures()`, which wraps each `config('mail.mailers')` entry's transport (each in its own `try/catch` + `report()` so one misconfigured mailer can't break boot). On-demand mailers (`Mail::mailer([...])`) are **not** wrapped.
...
`MailLog` is an **unguarded** ULID model (table from `config('mailog.table')`) with `pending()` / `sent()` / `failed()` scopes and json casts. It is unguarded so host apps can add real columns (e.g. `tenant_id`) via the published migration and have the data resolver fill them — there is no catch-all `data` column. `Status` (`PENDING` / `SENT` / `FAILED`) mixes in the `Enum` trait; `FAILED` is set by capture when the transport throws at send time (storing the error in the `error` column + `failed_at`), and hosts may also set it from bounce webhooks.
